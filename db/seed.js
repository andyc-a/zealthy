...
async function seed() {
  const conn = await mysql.createConnection({
    ...
    database: process.env.DB_NAME || 'zealthy_emr',
  ...
  for (const med of data.medications) {
    await conn.query('INSERT IGNORE INTO medications (name) VALUES (?)', [med]);
  }
  console.log(`Seeded ${data.medications.length} medications.`);

  for (const dose of data.dosages) {
    await conn.query('INSERT IGNORE INTO dosages (value) VALUES (?)', [dose]);
  }
  ...
  for (const user of data.users) {
    ...
    await conn.query(
      'INSERT INTO users (id, name, email, password) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE name=VALUES(name), email=VALUES(email)',
    ...
    for (const appt of user.appointments) {
      ...
      await conn.query(
        'INSERT INTO appointments (id, user_id, provider, datetime, `repeat`) VALUES (?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE provider=VALUES(provider), datetime=VALUES(datetime), `repeat`=VALUES(`repeat`)',
    ...
    for (const rx of user.prescriptions) {
      await conn.query(
        'INSERT INTO prescriptions (id, user_id, medication, dosage, quantity, refill_on, refill_schedule) VALUES (?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE medication=VALUES(medication), dosage=VALUES(dosage)',
